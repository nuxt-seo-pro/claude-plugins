...
1. Use `Glob` to check if `.claude/context/` exists
2. Use `Read` to load existing artifacts
3. **Validate format and staleness** before using
4. Use `AskUserQuestion` when critical info is missing or stale
5. Use `Write` to create/update artifacts
6. Always note artifact location in output to user
7. **Update `Last updated:` date** when modifying artifacts
